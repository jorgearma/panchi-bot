...
class pedido:
    ...
    def enviar_comanda_a_cocina(self):
        contenido_serializado = json.dumps(self.contenido_pedido)
            ...
            with conn.cursor() as cur:
                try:
                    cur.execute("""
                        ...
                        VALUES (?, ?);
                    """, (self.id_pedido, contenido_serializado))
                    conn.commit()
                    print(f"Comanda #{self.id_pedido} enviada a la cocina: {self.contenido_pedido}")
