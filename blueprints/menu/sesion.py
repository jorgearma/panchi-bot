...
def register(bp):
    ...
    def mostrar_confirmacion():
        ...
        pedido = menu_session.leer_sesion_confirmacion(pedido_id)
        if not pedido:
            logger.warning("CONFIRMACION_EXPIRADA  pedido_id=%.8s  ip=%s", pedido_id or "-", request.remote_addr)
            return jsonify({"error": "Pedido expirado o no encontrado"}), 404

        logger.info("CONFIRMACION_ACCESO  user_id=%s  ip=%s", pedido.get("userID", "-"), request.remote_addr)
    ...
    def mostrar_confirmacion_depago():
        ...
        pedido = menu_session.leer_sesion_confirmacion(pedido_id)
        if not pedido:
            logger.warning("PAGO_CONFIRMADO_EXPIRADO  pedido_id=%.8s  ip=%s", pedido_id or "-", request.remote_addr)
            return jsonify({"error": "Pedido expirado o no encontrado"}), 404

        logger.info("PAGO_CONFIRMADO_ACCESO  user_id=%s  ip=%s", pedido.get("userID", "-"), request.remote_addr)
