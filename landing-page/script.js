...
let phoneLeft, phoneRight;

function initPhoneParallax() {
  phoneLeft  = document.querySelector('.hero-phone--left');
  phoneRight = document.querySelector('.hero-phone--right');

  if (!phoneLeft || !phoneRight || window.innerWidth < 769) return;

  // After entrance animation completes, switch to floating
  setTimeout(() => {
    phoneLeft.classList.add('floating');
  }, 1400);
  setTimeout(() => {
    phoneRight.classList.add('floating');
  }, 1600);
...
function applyParallax(mx, my) {
  if (!phoneLeft || !phoneRight) return;
  ...
  // Apply as CSS custom properties (the floating animation handles base transform)
  phoneLeft.style.setProperty('--px', leftX + 'px');
  phoneLeft.style.setProperty('--py', leftY + 'px');
  phoneRight.style.setProperty('--px', rightX + 'px');
  phoneRight.style.setProperty('--py', rightY + 'px');

  // Shadow movement
  const shadowL = heroSection.querySelector('.hero-phone-shadow--left');
  const shadowR = heroSection.querySelector('.hero-phone-shadow--right');
  if (shadowL) shadowL.style.transform = `translateX(${-320 + mx * -6}px)`;
  if (shadowR) shadowR.style.transform = `translateX(${140 + mx * 6}px)`;
}

// For mobile: just start floating immediately
function initMobilePhones() {
  phoneLeft  = document.querySelector('.hero-phone--left');
  phoneRight = document.querySelector('.hero-phone--right');
  if (!phoneLeft || !phoneRight) return;

  // Make visible immediately on mobile
  phoneLeft.style.opacity = '1';
  phoneRight.style.opacity = '1';

  setTimeout(() => { phoneLeft.classList.add('floating'); }, 500);
  setTimeout(() => { phoneRight.classList.add('floating'); }, 700);
}
