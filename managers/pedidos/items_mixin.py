...
class GestorPedidosItemsMixin:
    ...
    def cancelar_pedido(self, pedido_id: int, motivo: str, empleado_id: int = None) -> tuple:
        ...
        if motivo not in self._MOTIVOS_CANCELACION:
            return (
                False,
                f"Motivo inválido. Válidos: {', '.join(sorted(self._MOTIVOS_CANCELACION))}",
                None,
            )
        ...
        try:
            ...
            s.add(
                AuditLog(
                    ...
                    detalles=json.dumps(
                        {'motivo': motivo, 'estado_anterior': estado_anterior}
                    ),
                )
            )
            s.commit()
    ...
    def eliminar_item(self, pedido_id: int, detalle_id: int, empleado_id: int = None) -> tuple:
        ...
        try:
            ...
            s.rollback()
            logger.error(
                "Error al eliminar item %s del pedido %s: %s",
                detalle_id,
                pedido_id,
                e,
            )
    ...
    def sustituir_item(
        ...
        try:
            ...
            sustituto = s.query(Producto).filter_by(
            ...
            qty = (
                cantidad_a_sustituir
                if cantidad_a_sustituir is not None
            ...
            msg = (
                ...
                + (
                    f" (quedan {valor_anterior['cantidad'] - qty}× originales)"
                    if parcial
                    else ""
                )
            ...
            s.rollback()
            logger.error(
                "Error al sustituir item %s del pedido %s: %s",
                detalle_id,
                pedido_id,
                e,
            )
